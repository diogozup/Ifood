package com.pandapanda.ifood.adapter;
...
public class AdapterPedido extends RecyclerView.Adapter<AdapterPedido.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int i) {
        ...
        for( ItemPedido itemPedido : itens ){
            ...
            descricaoItens += numeroItem + ") " + nome + " / (" + qtde + " x €ur " + preco + ") \n";
            numeroItem++;
        }
        descricaoItens += "Total: €ur " + total;
        holder.itens.setText(descricaoItens);

        int metodoPagamento = pedido.getMetodoPagamento();
        String pagamento = metodoPagamento == 0 ? "Dinheiro" : "Multibanco" ; // se tiver mais tipos de pagamentos, faco antes um switch-case
